...
public class ItemSearchServiceImpl implements ItemSearchService {
    ...
    //查询索引库
    //$scope.searchMap={'keywords':'手机','category':'手机','brand':'三星','spec':{'网络':3G,'内存':64G},'price':'','pageNo':1,'pageSize':40,'sort':'','sortField':''};
    ...
    public Map<String, Object> searchHighlightPageByKeywords(Map<String, String> searchMap) {
        ...
        if (null != searchMap.get("price") && !"".equals(searchMap.get("price"))) {
            ...
            if (p[1].equals("*")) {// searchMap.get(price).constats("*")
                fq = new SimpleFilterQuery(new Criteria("item_price").greaterThanEqual(p[0]));
            } else {
                fq = new SimpleFilterQuery(new Criteria("item_price").between(p[0], p[1]));
            }
